...
var imageSchema = new mongoose.Schema({
  ...
  width: {
    ...
    min: [64, 'Minimum width allowed is 64']
  },
  height: {
    type: mongoose.Schema.Types.Number,
...
imageSchema.virtual('size').get(function() {
...
function initialize(dbUri) {
  ...
  // init guest user
  db.Image.findOne({}).exec(function (err, image) {
    if (err) {
      ...
      db.Image.create({
        ...
        if (err)
          ...
          console.info('global image created: ', image);
        }
      });
    }
  });  
